...
async function loadDatabases(dbName: string) {
  try {
    ...
    ElMessage.error(`${$t('database-manager.loadDatabaseListFailed')}: ${error}`);
    return [];
  }
}
...
function handleCreateSchema() {
  if (contextMenuNode.value) {
    ...
    ElMessageBox.prompt($t('database-manager.enterSchemaName'), $t('database-manager.createSchema'), {
      ...
      .then(async ({ value: schemaName }) => {
        try {
          ...
          if (result.success) {
            ElMessage.success($t('database-manager.schemaCreatedSuccess', { schemaName }));
            // 刷新数据库节点
            await handleRefreshNode();
          } else {
            ElMessage.error(result.message || $t('database-manager.createSchemaFailed'));
          }
        } catch (error: any) {
          console.error('创建Schema失败:', error);
          ElMessage.error(error.message || $t('database-manager.createSchemaFailed'));
        }
      })
      .catch(() => {
        // 用户取消
...
function getContextMenuItems() {
  ...
  if (type === 'connection') {
    return [
      { label: $t('database-manager.refreshConnection'), icon: RefreshCw, action: handleRefreshNode },
      { label: $t('database-manager.viewInfo'), icon: Eye, action: handleViewInfo },
      {
  ...
  if (type === 'database') {
    ...
    const menuItems = [
      { label: $t('database-manager.refreshDatabase'), icon: RefreshCw, action: handleRefreshNode },
      { label: $t('database-manager.viewInfo'), icon: Eye, action: handleViewInfo },
    ];
    ...
    menuItems.push(
      ...
      { label: $t('database-manager.executeSql'), icon: Play, action: handleExecuteSQL },
  ...
  if (type === 'schema') {
    return [
      { label: $t('database-manager.refreshSchema'), icon: RefreshCw, action: handleRefreshNode },
      { label: $t('database-manager.viewInfo'), icon: Eye, action: handleViewInfo },
      {
        label: $t('database-manager.createNewTable'),
        icon: FilePlus,
  ...
  if (type === 'tables-folder') {
    return [
      { label: $t('database-manager.refreshTableList'), icon: RefreshCw, action: handleRefreshNode },
      {
        label: $t('database-manager.createNewTable'),
        icon: FilePlus,
        action: handleCreateTable,
  ...
  if (type === 'views-folder') {
    return [
      { label: $t('database-manager.refreshViewList'), icon: RefreshCw, action: handleRefreshNode },
      {
  ...
  if (type === 'table') {
    return [
      { label: $t('database-manager.viewTableStructure'), icon: Eye, action: handleViewInfo },
      {
      ...
      { label: $t('database-manager.executeSql'), icon: Play, action: handleExecuteSQL, divided: true },
      // { label: '导出表数据', icon: Download, action: handleExport },
      // { label: '导入数据', icon: Upload, action: handleImport },
      // { label: '清空表数据', icon: Trash, action: handleTruncateTable, danger: true, divided: true },
      { label: $t('database-manager.copyTableName'), icon: Copy, action: handleCopyName, divided: true },
      { label: $t('database-manager.refresh'), icon: RefreshCw, action: handleRefreshNode },
    ];
  }
  ...
  if (type === 'view') {
    return [
      { label: $t('database-manager.viewViewStructure'), icon: Eye, action: handleViewInfo },
      {
        label: $t('database-manager.queryData'),
        icon: Search,
        action: handleQueryData,
      ...
      { label: $t('database-manager.viewDefinitionSQL'), icon: FileText, action: handleViewDefinition },
      { label: $t('database-manager.executeSql'), icon: Play, action: handleExecuteSQL, divided: true },
