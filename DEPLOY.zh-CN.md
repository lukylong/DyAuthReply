...
| Django ASGI (uvicorn) | 8000 | 通过 Nginx 反代 | `/api/*`、`/ws/*` |
...
| `VITE_GLOB_API_URL` | `https://fastapi.zq-platform.cn/basic-api/` | **必须**改成你自己的后端 API 地址，例如 `https://zq.example.com/api/` |
...
server {
    ...
    # Django admin / swagger docs 如果有对外需求再开
...
> **前端 `VITE_GLOB_API_URL` 必须改成 `https://zq.example.com/api/`**，与 Nginx 反代路径一致，否则浏览器跨域/404。
...
4. **首次扫码**：生产环境保持 `DOUYIN_WORKER_HEADLESS=True`，二维码会通过 WebSocket 推到前端 `/douyin/account` 页面，用抖音 APP 扫
...
| 登录接口 CORS / 跨域 | `VITE_GLOB_API_URL` 必须是 **同源 + /api/** 或后端 `CORS_ALLOWED_ORIGINS` 显式允许 |
| WebSocket 连不上（二维码不显示） | Nginx `/ws/` 的 `Upgrade` 头未透传；`proxy_read_timeout` 太短；服务器防火墙 |
