...
import {
  createKuaishouAccountGroup,
  deleteKuaishouAccountGroup,
  getKuaishouAccountGroupList,
  updateKuaishouAccountGroup,
} from '#/api/core/kuaishou';

defineOptions({ name: 'KuaishouAccountGroup' });

const rows = ref<KuaishouAccountGroup[]>([]);
...
const form = reactive<KuaishouAccountGroupInput>({
  name: '',
  color: '#FF6B00',
...
async function load() {
  loading.value = true;
  try {
    const res = await getKuaishouAccountGroupList({
      page: page.page,
...
function openEdit(row: KuaishouAccountGroup) {
...
async function onSave() {
  ...
  try {
    if (editingId.value) {
      await updateKuaishouAccountGroup(editingId.value, form);
      ElMessage.success('已更新');
    } else {
      await createKuaishouAccountGroup(form);
...
async function onDelete(row: KuaishouAccountGroup) {
  await ElMessageBox.confirm(`确定删除分组 "${row.name}" ?`, '提示', {
    type: 'warning',
  });
  try {
    await deleteKuaishouAccountGroup(row.id);
...
<template>
  <Page title="账号分组管理" description="按业务线/团队对多个快手账号进行分组管理">
