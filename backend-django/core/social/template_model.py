...
# -*- coding: utf-8 -*-
"""
@File: core/social/template_model.py
@Desc: Reply Template - 跨平台回复模板

可复用的"文本 + 链接 + 变量占位符"内容单元，规则 / 快捷回复均可引用。
支持变量插值：{{nickname}}、{{time_greeting}}、{{product_name}} 等。

platform 为空表示"通用模板"（所有平台可用）；指定值则该模板仅在对应平台展示/可选。
"""
...
class ReplyTemplate(RootModel):
    ...
    platform = models.CharField(
        ...
        blank=True,
        db_index=True,
        help_text="归属平台；为空表示通用模板（所有平台可用）",
    )

    name = models.CharField(max_length=64, help_text="模板名称", db_index=True)
