...
### 抖音私信自动回复模块（纯 HTTP 协议）

`core.douyin` 模块以纯 HTTP 协议直连抖音 imapi，无浏览器依赖；签名走 Node.js（`dy_ab.js` 算 a_bogus + bd-ticket-guard），请确保运行环境已安装 Node.js。

独立 worker 进程需要额外在 `env/*.py` 中配置：

```python
DOUYIN_DATA_DIR = "/var/lib/zq-platform/douyin"   # 加密凭证根目录
DOUYIN_STORAGE_ENCRYPTION_KEY = "<Fernet key>"    # 登录态加密密钥
DOUYIN_TRANSPORT_BACKEND = "http_protocol"        # 纯协议后端（默认）
DOUYIN_SIGN_BACKEND = "js"                        # JS 签名后端（默认）
